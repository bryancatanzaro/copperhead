    ...
    host_module.add_function(
        FunctionBody(
            ...
            Block([Statement(x) for x in [
                        ...
                        'sumScanInstance(input.devicePointer, input.length, output.devicePointer)',
                        'return']])))
    host_module.add_to_preamble([Include('cuarray.h'),
                                 Include('boost/python/extract.hpp')])

    device_module = codepy.cuda.CudaModule(host_module)
    device_module.add_to_preamble([Include('scan.h')])
    sumInstance = FunctionBody(
        FunctionDeclaration(Value('void', 'sumScanInstance'),
                            [Value('CUdeviceptr', 'inputPtr'),
                             Value('int', 'length'),
                             Value('CUdeviceptr', 'outputPtr')]),
        Block([Statement(x) for x in ['sum_scan<%s>((%s*)inputPtr, length, (%s*)outputPtr)' % (result_type, result_type, result_type),
               'return']]))
    ...
    host_module.add_function(
        FunctionBody(
            ...
            Block([Statement(x) for x in [
                        ...
                        'CuArray input = extractArray(inputArray)',
                        'CuArray output = extractArray(outputArray)',
                        'scanInstance(input.devicePointer, input.length, output.devicePointer)',
                        'return']])))
    host_module.add_to_preamble([Include('cuarray.h'),
                                 Include('boost/python/extract.hpp')])

    device_module = codepy.cuda.CudaModule(host_module)
    device_module.add_to_preamble([Include('wrappers/scan.h')])
    if user_functor:
        device_module.add_to_module([Line(functor)])
    else:
        device_module.add_to_preamble([Include('operators.h')])
    scanInstance = FunctionBody(
        FunctionDeclaration(Value('void', 'scanInstance'),
                            [Value('CUdeviceptr', 'inputPtr'),
                             Value('int', 'length'),
                             Value('CUdeviceptr', 'outputPtr')]),
        Block([Statement(x) for x in ['scan<%s, %s>(%s(), (%s*)inputPtr, length, (%s*)outputPtr)' % (result_type, functor_name, functor_name, result_type, result_type),
                                      'return']]))
