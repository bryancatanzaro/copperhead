...
@cubox
@cutype("([a], [Int]) -> [a]")
def gather(source, indices, cache):
    source.using_remote()
    ...
    host_module.add_function(
        FunctionBody(
            ...
            Block([Statement(x) for x in [
                        ...
                        'gatherInstance(cuX.devicePointer, cuI.devicePointer, cuI.length, cuO.devicePointer)',
                        'return']])))
    ...
    device_module.add_to_preamble([Include('wrappers/gather.h')])
    gatherInstance = FunctionBody(
        FunctionDeclaration(Value('void', 'gatherInstance'),
                            [Value('CUdeviceptr', 'x'),
                             Value('CUdeviceptr', 'i'),
                             Value('int', 'length'),
                             Value('CUdeviceptr', 'o')]),
        Block([Statement(x) for x in [
                    'gather<%s>((int*)i, (%s*)x, length, (%s*)o)' % (result_type, result_type, result_type),
                    'return']]))
    device_module.add_function(gatherInstance)
